...
export default async function handler(req, res) {
  ...
  // Debug endpoint - GET shows env var status
  if (req.method === 'GET') {
    ...
    return res.status(200).json({
      ...
      hasAnonKey: !!anonKey,
      envKeys: Object.keys(process.env).filter(k => k.includes('SUPA') || k.includes('VITE')).sort(),
    });
  }
  ...
  if (!supabaseUrl || !serviceRoleKey || !anonKey) {
    return res.status(500).json({ error: 'Server misconfigured', missing: { url: !supabaseUrl, serviceKey: !serviceRoleKey, anonKey: !anonKey } });
  }
  ...
  try {
    ...
    if (!insertRes.ok) {
      ...
      return res.status(500).json({ error: 'Failed to create code', detail: err });
    }
